...
#else
  return (SORT_TYPE*)ta_alloc(size * sizeof(SORT_TYPE));
#endif
}
...
static void TIM_SORT_RESIZE(TEMP_STORAGE_T *store, const size_t new_size) {
  if ((store->storage == NULL) || (store->alloc < new_size)) {
    ...
    if (tempstore == NULL) {
      /*TODO This needs to exit if something bad happens, maybe print out an error to the screen
...
void SQRT_SORT(SORT_TYPE *arr, size_t Len) {
  ...
  Tags = (int*)ta_alloc(NK * sizeof(int));
